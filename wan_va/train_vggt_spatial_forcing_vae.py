...
# Copied from train_vggt_spatial_forcing.py patterns; does NOT patch load_transformer
# (standard WanTransformer3DModel, no intermediate hidden hook on the diffusion trunk).
...
from .configs.va_robotwin_train_vggt_spatial_forcing_vae_cfg import (
    va_robotwin_train_vggt_spatial_forcing_vae_cfg,
)
from .configs.va_robotwin_train_vggt_spatial_forcing_vae_debug_cfg import (
    va_robotwin_train_vggt_spatial_forcing_vae_debug_cfg,
)
...
class VGGTSpatialForcingVaeTrainer(base_train.VGGTTrainer):
    ...
    def train(self):
        ...
        progress_bar = tqdm(
            ...
            desc="Training (SpatialForcingVAE)",
...
SPATIAL_FORCING_VAE_CONFIGS = dict(BASE_CONFIGS)
SPATIAL_FORCING_VAE_CONFIGS['robotwin_train_vggt_spatial_forcing_vae'] = (
    va_robotwin_train_vggt_spatial_forcing_vae_cfg
)
SPATIAL_FORCING_VAE_CONFIGS['robotwin_train_vggt_spatial_forcing_vae_debug'] = (
    va_robotwin_train_vggt_spatial_forcing_vae_debug_cfg
)


def run(args):
    config = SPATIAL_FORCING_VAE_CONFIGS[args.config_name]
...
def main():
    parser = argparse.ArgumentParser(
        description="Train WAN with VGGT + Wan VAE encoder / VGGT feature alignment"
    )
    parser.add_argument("--config-name", type=str, default='robotwin_train_vggt_spatial_forcing_vae')
