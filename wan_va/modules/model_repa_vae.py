...
"""Wan transformer with intermediate-layer hooks for VAE-REPA / similar
representation alignment objectives.

This file is intentionally a *separate copy* of the spatial_forcing
backbone, so that VAE-REPA experiments can evolve independently from
the spatial-forcing branch while keeping the same hook API:

    transformer(input_dict_with_extra_keys, train_mode=True)

Extra keys understood by ``forward_train`` (all optional):

    return_hidden_layers : bool
        If True, in addition to ``(latent_pred, action_pred)`` the call
        returns a dict::
            {
              'pred':                (latent_pred, action_pred),
              'align_hidden_states': dict[int, Tensor]  /  Tensor  /  None,
            }
    align_layer_idx : int | list[int] | tuple[int] | set[int] | None
        Block indices whose intermediate hidden states should be captured.
        When a single int is passed, ``align_hidden_states`` is a single
        Tensor of that layer's hidden; when a list/tuple/set is passed it
        is a dict ``{block_idx: Tensor}``.
    align_video_tokens_only : bool, default True
        If True, only the *video* token slice is captured (i.e. the
        ``latent_hidden_states`` portion before any condition/action/pad
        slices). Tensor is reshaped to ``(B, L, C)``.

This is exactly the same contract as ``model_spatial_forcing.py``, so any
trainer that already speaks that protocol (e.g. spatial_forcing /
geometry_forcing) can swap in this backbone unchanged.
"""
import torch
...
class WanTransformer3DModel(BaseWanTransformer3DModel):
    """Wan diffusion transformer with optional per-block hidden capture."""

    def forward_train(self, input_dict):
        # bf16 cast for the latent / action streams (parity with the base
        # model_spatial_forcing variant; required because some of the
        # downstream patched attention paths assume bf16).
        ...
        # Pad to multiple of 128 for FlexAttention block size.
        ...
        # ---- intermediate-layer hidden capture (REPA-style hook) ----
        ...
        # Final norm + per-stream split / projections.
        ...
        # When a single int was requested, return that layer's tensor directly
        # (else keep the dict). Keep the same convention as model_spatial_forcing.
